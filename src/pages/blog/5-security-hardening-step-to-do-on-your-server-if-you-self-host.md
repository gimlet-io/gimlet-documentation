...
Like DHH argued recently, [running a server should not be mystified](https://world.hey.com/dhh/dare-to-connect-a-server-to-the-internet-01d25a07) when you’d like to use software. But there are a few things we believe you should do on your server to increase its security.
...
Budget-friendly VPS providers are great in case you need a server connected to the internet. They’re convenient, don’t require as much effort and money to set up as a Virtual Private Cloud (VPC).

VPCs are good for a couple of reasons, flexibility and security for example. But this requires lots of planning and effort to execute, including configuring network components, securing them, integrating them to your on-premises network when needed, tracking your network’s costs, and maintaining the network. Major providers offer an abundance of shortcuts to setting up a VPC - part of the reason why AWS bills going bonkers became a meme a while ago.
...
For this post, we chose Hetzner (not affiliated), but most of the work that needs to be done to secure the server will be unrelated to providers. We also chose Ubuntu Linux 22.04 as it’s the most common option available. Essentially you'll be able to follow these directions at any provider that offers Ubuntu 22.04 as an operating system for your VPS.

## Securing access

Now that you have your VPS ready, you can use an SSH terminal ([PuTTY](https://www.putty.org/) on Windows, terminal on macOS and Linux) to connect to it. Use your root user and the generated password from your password manager.
...
The console will ask for a password for the user, enter one. If you don’t see any characters showing up when you type, don’t worry, it’s a security measure to hide the number of characters.

**Pro tip:** always use a password manager and secure, high quality passwords.
...
Find and uncomment the following lines by deleting the `#` from in front of their respective lines. This will disable root access and password access to your server while only enabling the usage of the public key you saved a step earlier:
...
**Pro tip:** whitelist your home or office IP to avoid banning yourself. You can do this with the command below. Replace `<yourip>` with your IP address:
...
Setting up a VPN is a difficult deal but [Tailscale](https://tailscale.com/) is one of the easiest tools to set up. To install Tailscale on your Ubuntu 22.04 VPS, follow the directions below.

To make sure you'll get Tailscale from a credible source, add their GPG key and repository by running the commands below:
...
Run the Tailscale install script below:
...
Check if everything works fine by verifying your IP address. Run:

```
tailscale ip -4
```

It's recommended for servers to turn off key expiry. You can do so by following their [documentation](https://tailscale.com/kb/1028/key-expiry#disabling-key-expiry)
...
Backing up a server is a tedious task that can take up a lot of disk space. However creating a snapshot of a VPS is a one button action that backs up your whole server as-is.

If you'd like to take a snapshot of your VPS at Hetzner, check out their [documentation](https://docs.hetzner.com/cloud/servers/getting-started/taking-snapshots/). The only downside of snapshots is that they need to be executed manually.
